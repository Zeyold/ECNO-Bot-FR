...
import { localizeText } from './frenchLocalization.js';
...
function localizeResponseOptions(options) {
    if (typeof options === 'string') return localizeText(options);
    if (!options || typeof options !== 'object') return options;
    return typeof options.content === 'string'
        ? { ...options, content: localizeText(options.content) }
        : options;
}
        ...
        interaction.reply = async (options) => {
            ...
            if (!interaction.deferred && !interaction.replied) {
                if (coordinator && interaction._isPrefixCommand) {
                    return coordinator.respond(localizeResponseOptions(options));
                }
                return await originalReply(localizeResponseOptions(options));
            }

            if (interaction.deferred && !interaction.replied) {
                if (coordinator && interaction._isPrefixCommand) {
                    return coordinator.edit(sanitizeEditReplyOptions(localizeResponseOptions(options)));
                }
                return await originalEditReply(sanitizeEditReplyOptions(localizeResponseOptions(options)));
            }

            if (coordinator && interaction._isPrefixCommand) {
                return coordinator.followUp(localizeResponseOptions(options));
            }
            return await originalFollowUp(localizeResponseOptions(options));
        };
